...
================================================================================================*/
int AlarmRecordInfo::AlarmRecordReset(FILE* fp,INT16U iLen,INT8U iItemLen)
{
	KL_WARN("Will reset Alarm Data\n");
	AlarmData[0]=0x00;
	fseek(fp,0,SEEK_SET);
	fwrite(AlarmData,1,1,fp);
	fseek(fp,1,SEEK_SET);
	fwrite(AlarmData,1,1,fp);
	unsigned char date_tmp[CONST_ALARM_RECORD_ITEM_LENGTH * 10];
	memset(date_tmp, 0xff, CONST_ALARM_RECORD_ITEM_LENGTH * 10);
	for (int i = 0; i < CONST_ALARM_FILE_LENGTH/10; i++)
	{
		fseek(fp, 2 + CONST_ALARM_RECORD_ITEM_LENGTH * 10 * i, SEEK_SET);
		fwrite(date_tmp, CONST_ALARM_RECORD_ITEM_LENGTH * 10, 1, fp);
	}
	if (CONST_ALARM_FILE_LENGTH % 10 != 0)		//如果不是10的整数倍，需要将余下的进行单独初始化，避免循环判断造成资源浪费
	{
		fseek(fp, 2 + CONST_ALARM_RECORD_ITEM_LENGTH * 10 * (CONST_ALARM_FILE_LENGTH/10), SEEK_SET);
		fwrite(date_tmp, CONST_ALARM_RECORD_ITEM_LENGTH * (CONST_ALARM_FILE_LENGTH % 10), 1, fp);
	}
	return 0;
}

int AlarmFile::AddAlarmRecord(const INT8U *iRecordContent,INT8U iTrueLen,INT16U iAlarmNo,INT16U iMeterNo,INT8U iItemLen,INT16U FileLen,char *FileName)
{
	char bcdtime[7];
	Get_CurBCDTime7(bcdtime);
	return AddAlarmRecord(iRecordContent,iTrueLen,iAlarmNo,bcdtime,iMeterNo,iItemLen,FileLen,FileName);
}
...
=============================================================================================*/
int AlarmFile::AddAlarmRecord(const INT8U* iRecord,INT8U iTrueLen,INT16U iAlarmNo,const char *bcdtime,INT16U iMeterNo,INT8U iItemLen,INT16U FileLen,char *FileName)
{
	...
	if (fp == NULL)
	{
		...
		recordInfo.Init(fp1,CONST_ALARM_FILE_LENGTH,CONST_ALARM_RECORD_ITEM_LENGTH);
		FileBase::Close(fp1);
		fp = FileBase::OpenFile(FileName,"rb+");
		if (fp == NULL)
		{
	...
	writenIndex++; 
	if (writenIndex == CONST_ALARM_FILE_LENGTH)
	{
		writenIndex = 0;
	}
	...
	record[10] = bcdtime[4];
	record[11] = bcdtime[5];
	record[12] = (INT8U)iAlarmNo;
	record[13] = (INT8U)(iAlarmNo>>8);
	memcpy(record+14,iRecord,iTrueLen);
	//fseek(fp,2+(fileLen-1)*iItemLen,SEEK_SET);
	fseek(fp,4+writenIndex*iItemLen,SEEK_SET);

	bstring alarmdata;
	alarmdata.append(record,iItemLen);
	KL_INFO_DUMP(dump);
	dump<<"Alarm Log Data:"<<alarmdata<<"\n";
	
	fwrite(record,iItemLen,1,fp);	
	delete[] record;
	CONTEXT.httx_sendBuffer.Alarm_report_flag=1;
	return 0;
}

//返回-2表示该条记录不存在，返回-1表示文件不存在，返回0表示成功
int AlarmFile::SetAlarmRecordReaded(INT16U io_Offset,INT8U iItemLen,INT16U FileLen,char *FileName)
{
	if (io_Offset == CONST_ALARM_RECORD_OFFSET || io_Offset >= FileLen)
	{
		return -2; 
	}
...
int AlarmFile::SetAlarmRecordReaded(FILE* fp,INT16U io_Offset,INT8U iItemLen,INT16U FileLen)
{
	if (io_Offset == CONST_ALARM_RECORD_OFFSET || io_Offset >= FileLen)
	{
		return -2; 
...
//io_Offset由调用的程序传入，并返回给调用的程序，以供SetAlarmRecordReaded用于定位读取的记录的目标地址
int AlarmFile::GetAlarmRecord(bstring &iRecord,INT8U &i_TrueLen,INT16U &io_Offset,INT8U iItemLen,INT16U iFileLen,char *FileName)
{
	INT8U record[CONST_ALARM_RECORD_ITEM_LENGTH];
...
//i_TrueLen由调用的程序传入iRecord数组的长度，该函数将使用该形参来返回该条记录的实际长度，应用程序应该根据此长度来打包上送帧
//io_Offset由调用的程序传入，并返回给调用的程序，以供SetAlarmRecordReaded用于定位读取的记录的目标地址
int AlarmFile::GetAlarmRecord(INT8U *iRecord,INT8U &i_TrueLen,INT16U &io_Offset,INT8U iItemLen,INT16U iFileLen,char *FileName)
{
	int iResult = -1;
	FILE *fp = FileBase::OpenFile(FileName,"rb");
	if (fp == NULL)
	...
	INT8U flag = 0x00;
	INT8U AlarmNo[4] = {0,0,0,0};
	for (INT16U i = 0;i != iFileLen;i++)//读取逐条记录的标志位，假如该标志位为1则立刻停止搜索
	{
		FileBase::ReadBinFile(fp,&flag,4 + i*iItemLen,SEEK_SET,1);
		if (flag == 1)
		{
			FileBase::ReadBinFile(fp,iRecord,4 + i*iItemLen,SEEK_SET,CONST_ALARM_RECORD_ITEM_LENGTH);
			AlarmNo[0] = iRecord[12];
			AlarmNo[1] = iRecord[13];
			AlarmNo[2] = iRecord[14];
			iResult = GetTrueLen(AlarmNo,i_TrueLen);
	...
	//SetAlarmRecordReaded(io_Offset);//只要读取了，就认为该条记录已经上报过了
	return iResult;//返回0表示读取成功
}

int AlarmFile::GetTrueLen(INT8U *iAlarmNo,INT8U &i_TrueLen)
{
	int iResult = -1;
	if (iAlarmNo[1] == 0x01)
	{
...
int AlarmFile::GetAlarmRecords(bstrings &iRecords,INT8U irCount,time_t tBeginTime,time_t tEndTime,INT8U iItemLen,INT16U iFileLen,char *FileName)
{
	int iResult = 0;
	//KL_INFO("打开告警数据文件：%s\n",FileName);
	INT8U record[CONST_ALARM_RECORD_ITEM_LENGTH];
	...
	bstring bstrRecord;
	if (iFileLen > CONST_ALARM_FILE_LENGTH)
	{
		iFileLen = CONST_ALARM_FILE_LENGTH;
		KL_INFO("iFileLen = %d\n",iFileLen);
	}
	KL_INFO("iFileLen = %d\n",iFileLen);
	...
	for (INT16U i = 0;i != iFileLen; i++)
	{
		...
		{
			AlarmNo[0] = record[12];//反序保存
			AlarmNo[1] = record[13];
			AlarmNo[2] = record[14];
			GetTrueLen(AlarmNo,i_TrueLen);
			if (i_TrueLen <= 0)
			{
				continue;
			}
	...
	//		KL_INFO_DUMP(dump);
		//	dump<<"bstrRecord:"<<bstrRecord<<"\n";
			...
			iResult += 1;
			//KL_VERBOSE("取出第%d条数据，告警时间：%02X%02X-%02X-%02X %02X:%02X:%02X，告警编码：%02X%02X\n",iResult,bcdtime[0],bcdtime[1],bcdtime[2],bcdtime[3],bcdtime[4],bcdtime[5],bcdtime[6],AlarmNo[1],AlarmNo[0]);
...
int AlarmFile::GetAlarmRecords(bstrings &iRecords,INT8U irCount,char *tBeginTimeBCD7,char *tEndTimeBCD7,INT8U iItemLen,INT16U iFileLen,char *FileName)
{
...
//int AlarmFile::ConvertMeterNoToBCD(INT16U iMeterNo,INT8U *iBcdMeterNo)
//{
...
//int AlarmFile::AddAlarmRecord(bstring &iRecord,INT8U iTrueLen,INT8U iItemLen,INT16U FileLen,char *FileName)
//{
//	return AddAlarmRecord((INT8U *)iRecord.c_str(),iTrueLen,iItemLen,FileLen,FileName);
//}
//
