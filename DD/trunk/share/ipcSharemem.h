...
#define FILE_IPC_SHAREMEM MNT_PATH_BASE"/ramdisk/ipcsharemem.dat"	//share memery file
...
struct SIpcRealControlCommandQueue
{
	...
	SIpcRealControlCommand m_RealCtlCmd[CONS_REAL_CONTROL_COMMAND_MAX_COUNT];
...
struct SIpcNorTask
{
	...
	SIpcPlan m_Plans[CONS_TASKPLAN_MAX_COUNT];//执行方案
};

struct SIpcNorTasks
{
	SIpcNorTask m_Tasks[CONS_NORTASK_MAX_COUNT];/* 任务集合 */
};
...
struct SIpcLuxTask
{
	...
	INT32U m_LuxSensorsBS;//方案相关传感器有效位，bit0对应sensor1~bit31对应sensor32，目前只用了bit0~bit15对应1~16号sensor
 	SIpcLuxPlan m_Plans[CONS_LUXTASKPLAN_MAX_COUNT];
};
struct SIpcLuxTasks
{
	SIpcLuxTask m_LuxTasks[CONS_LUXTASK_MAX_COUNT];/* 光控任务集合 */
};
/* 
brief:恒照任务数据结构
*/
...
{
	SIpcConstantLuxTask m_ConstantLuxTasks[CONS_CONSTANTLUXTASK_MAX_COUNT];/* 恒照任务集合 */
};
...
struct SIpcClLeakInfo
{
	...
	INT16U m_DelayMs;
	INT16S m_DtVol;//漏电检测阀值电压
	INT8U m_CutEnable;//是否切电
};
